    ...
    try:
        ...
        while index < len(lines):
            ...
            if "GLOBAL" in line:
                index += 1
                if len(lines) < 3:
                    raise Exception("Failed to parse pool stats data")
                ...
                if cluster_size_idx == -1 or cluster_avail_idx == -1 or \
                    ...
                    raise Exception("Missing fields in cluster stat")
                index += 1
                if index >= len(lines):
                    raise Exception("No cluster stats to parse")
                line = lines[index]
        ...
        collectd.error("Exception %s" % str(ex))
        return cluster_stat, pool_stat
